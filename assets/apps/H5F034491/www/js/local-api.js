...
(function (global) {
  ...
  // ========== 大盘指数数据 ==========
  var INDEX_SNAPSHOTS_KEY = 'local_indexSnapshots';
  var INDEX_CODES = ['sh000001', 'sz399001', 'sz399006'];

  // 拉取大盘指数实时/收盘数据（同步 XHR）
  function fetchIndexData() {
    var cache = Store.get('local_indexQuotes', {});
    var result = {};
    try {
      INDEX_CODES.forEach(function (code) {
        var url = '/proxy/http://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=' + code + ',day,,,1,qfq';
        var xhr = new XMLHttpRequest();
        xhr.open('GET', url, false);
        xhr.setRequestHeader('Referer', 'http://gu.qq.com/');
        try { xhr.send(null); } catch (e) {}
        if (xhr.status === 200) {
          try {
            var data = JSON.parse(xhr.responseText);
            var klines = (data.data && data.data[code] && data.data[code].day) || [];
            if (klines.length > 0) {
              var last = klines[klines.length - 1];
              var close = parseFloat(last[2]);
              var open_ = parseFloat(last[1]);
              var prevClose = parseFloat(last[7] || open_);
            ...
            if (cache[code]) result[code] = cache[code];
          }
        } else if (cache[code]) {
          result[code] = cache[code];
        }
      ...
      console.warn('[LocalAPI] 大盘行情拉取失败，使用缓存:', e);
    }
    Store.set('local_indexQuotes', cache);
    INDEX_CODES.forEach(function (code) {
      if (!result[code]) {
        result[code] = cache[code] || { close: 0, open: 0, high: 0, low: 0, prevClose: 0, changePct: 0 };
      }
    });
    return result;
  }

  function saveIndexSnapshot(date, indexData) {
    var all = Store.get(INDEX_SNAPSHOTS_KEY, {});
    all[date] = indexData;
    Store.set(INDEX_SNAPSHOTS_KEY, all);
  }

  function getIndexSnapshot(date) {
    var all = Store.get(INDEX_SNAPSHOTS_KEY, {});
  ...
  function takeDailySnapshot(username) {
    ...
    transactionsFor(username).forEach(function(t) {
      ...
      // 构建当日交易明细
      var code = normalizeCode(t.code || t.jycode);
      var posEntry = account.positions.find(function(p) { return normalizeCode(p.code) === code; });
      var zongchengben = posEntry ? num(posEntry.unicbze) : gross;
      var dryk = posEntry ? num(posEntry.yingkui) : 0;
      datas.push({
        code: code,
        dryk: round(dryk, 4),
        zongchengben: round(zongchengben, 4),
        zdrykzdryk: round(dryk, 4)
      });
    });

    // 拉取大盘指数数据
    var indexData = fetchIndexData();
    saveIndexSnapshot(today, indexData);
    ...
    var snap = {
      ...
      datas: datas,
      sprescount: datas.length,
      sh000001: indexData.sh000001 || { close: 0, changePct: 0 },
      sz399001: indexData.sz399001 || { close: 0, changePct: 0 },
      sz399006: indexData.sz399006 || { close: 0, changePct: 0 },
      ts: Date.now()
    };
  ...
  function calculateAccount(username, source) {
    ...
    // 优先使用最后一条快照的 zzc 作为真实总资产
    var equity = cash + marketValue;
    try {
      var _snaps = getDailySnapshots(username);
      if (_snaps && _snaps.length > 0) {
        _snaps = _snaps.slice().sort(function(a, b) { return String(a.date).localeCompare(String(b.date)); });
        var _last = _snaps[_snaps.length - 1];
        if (_last && _last.zzc && num(_last.zzc) > 0) {
          equity = num(_last.zzc);
        }
      }
    } catch (ignore) {}
  ...
  function buildCalendarSummary(username) {
    ...
    // 按日期排序快照，计算每日盈亏
    snaps = snaps.slice().sort(function(a, b) { return String(a.date).localeCompare(String(b.date)); });
    var lastZzc = 0;
    snaps.forEach(function(snap) {
      var parts = snap.date.split('-');
      var year = parts[0];
      var month = parts[1];
      var zzc = num(snap.zzc);
      // snap.drjk 优先，否则用 zzc 差值
      var profit = snap.drjk !== undefined ? num(snap.drjk) : (lastZzc > 0 ? num(zzc - lastZzc) : 0);
      lastZzc = zzc;
  ...
  // 从快照和账户构建完全对齐原始服务器的 data[] 行
  function buildFullDataRow(snap, prevSnap, account, firstAsset, xuhao) {
    var initMoney = account.initial;
    var zzc = num(snap.zzc);
    var zsz = num(snap.sz);
    var zky = zzc - initMoney;
    var zdryk = prevSnap ? (zzc - num(prevSnap.zzc)) : 0;
    var prevZzc = prevSnap ? num(prevSnap.zzc) : firstAsset;
    var zdrykl = prevZzc > 0 ? zdryk / prevZzc : 0;

    // 大盘数据
    var shIdx = snap.sh000001 || { close: 0, changePct: 0 };
    var dp_zzc = round(num(shIdx.close), 4);
    var dpzhdie = round(num(shIdx.changePct), 4);
    var dpevfu = num(shIdx.changePct) / 100;

    // 累计收益率（相对起始资金）
    var first_ykl_val = initMoney > 0 ? zky / initMoney * 100 : 0;
    var first_ykl = round(first_ykl_val, 2);

    // 期初资产（区间起点的资产值，用于曲线基准）
    var first_zzc_MY = firstAsset || initMoney;

    return {
      zdryk: round(zdryk, 2).toFixed(2),
      zdrykl: round(zdrykl, 10),
      cdate: snap.date,
      zzc: round(zzc, 4),
      zsz: Math.round(zsz),
      zky: round(zky, 4),
      datas: snap.datas || [],
      sprescount: Math.round(snap.sprescount || (snap.datas ? snap.datas.length : 0)),
      dpevfu: round(dpevfu, 8),
      dp_zzc: dp_zzc,
      dpzhdie: dpzhdie,
      first_ykl: first_ykl,
      minfirykl: round(first_ykl_val, 10),
      first_zzc: round(initMoney, 2),
      xuhao: xuhao,
      first_zzc_MY: round(first_zzc_MY, 4)
    };
  }
  ...
  function calendarResponse(params) {
    ...
    // 找到上个月的最后一个快照作为 prevSnap 参考
    // 计算 filteredSnaps 中每条的完整 data 行
    var filteredSnaps = snaps.filter(function (snap) {
      return snap.date.indexOf(prefix) === 0;
    });

    // 构建完整 data 行，对齐原始服务器
    var dataRows = filteredSnaps.map(function (snap, idx) {
      var prevSnap = idx > 0 ? filteredSnaps[idx - 1] : null;
      var firstAsset = prevSnap ? num(prevSnap.zzc) : account.initial;
      return buildFullDataRow(snap, prevSnap, account, firstAsset, idx + 1);
    });

    // 如果今天在所选月份中且还没有快照，添加当前数据
    if (today.indexOf(prefix) === 0) {
      var hasToday = filteredSnaps.some(function (s) { return s.date === today; });
      if (!hasToday) {
        var lastSnap = filteredSnaps.length > 0 ? filteredSnaps[filteredSnaps.length - 1] : (snaps.length > 0 ? snaps[snaps.length - 1] : null);
        var tempSnap = {
          date: today,
          zzc: account.equity,
          sz: num(account.total.zsz),
          datas: [],
          sprescount: 0,
          sh000001: {},
          sz399001: {},
        ...
        var firstAsset = lastSnap ? num(lastSnap.zzc) : account.initial;
        dataRows.push(buildFullDataRow(tempSnap, lastSnap, account, firstAsset, dataRows.length + 1));
      }
    }

    var profit = dataRows.reduce(function (sum, row) { return sum + num(row.zdryk); }, 0);
    var maxDailyProfit = dataRows.length > 0 ? Math.max.apply(null, dataRows.map(function (r) { return Math.abs(num(r.zdryk)); })) : 0;
    var summary = buildCalendarSummary(username);

    // sylzhudata 各字段（common.js 里 response.sylzhudata.myykl 等访问依赖）
    var yStr = String(year);
    var mStr = pad2(month);
    var prevY = (mStr === '01' ? String(parseInt(yStr, 10) - 1) : yStr);
    var prevM = (mStr === '01' ? '12' : pad2(parseInt(mStr, 10) - 1));
    var prevPrefix = prevY + '-' + prevM + '-';
    var prevSnaps = snaps.filter(function(s){ return String(s.date).indexOf(prevPrefix) === 0; });
    var prevProfit = prevSnaps.reduce(function(sum, s){ return sum + num(s.drjk !== undefined ? s.drjk : 0); }, 0);
    var todaySnap = snaps.find(function(s){ return String(s.date) === today; });
    var chyk = todaySnap ? num(todaySnap.drjk) : 0;
    var weekDate = new Date(); weekDate.setDate(weekDate.getDate() - 6);
    var weekStart = weekDate.toISOString().slice(0, 10);
    var weekSnaps = snaps.filter(function(s){ return String(s.date) >= weekStart; });
    var weekProfit = weekSnaps.reduce(function(sum, s){ return sum + num(s.drjk !== undefined ? s.drjk : 0); }, 0);
    var paoying = '0.00%';
    ...
    return Object.assign(responseBase(params), {
      ...
      ykl: (account.initial ? profit / account.initial * 100 : 0).toFixed(2),
      dpykl: '0.00%',
      maxyk: String(round(maxDailyProfit, 2)),
      css: profit >= 0 ? 'pred' : 'pblue',
      szcss: 'pred',
      RiliData: summary,
      zhenglidata: {},
      paoying: paoying,
      sylzhudata: {
        myyklfs: (account.initial ? profit / account.initial * 100 : 0).toFixed(2) + '%',
        myykl: (account.initial ? profit / account.initial * 100 : 0).toFixed(2) + '%',
        shayklfs: (account.initial ? prevProfit / account.initial * 100 : 0).toFixed(2) + '%',
        shaykl: (account.initial ? prevProfit / account.initial * 100 : 0).toFixed(2) + '%',
        shyklfs: (account.initial ? weekProfit / account.initial * 100 : 0).toFixed(2) + '%',
        shykl: (account.initial ? weekProfit / account.initial * 100 : 0).toFixed(2) + '%',
        chyklfs: (account.initial ? chyk / account.initial * 100 : 0).toFixed(2) + '%',
        chykl: (account.initial ? chyk / account.initial * 100 : 0).toFixed(2) + '%',
      },
    });
  }
  ...
  function curveResponse(params) {
    ...
    // 计算周期和起始日期
    var zhouqiInput = params.zhouqi || params.type || '';
    var start = periodStart(zhouqiInput);
    var zhouqiMap = {
      'qxmonth': '近一月', 'monthly': '近一月', '1': '近一月', 'month': '近一月',
      'qxthree': '近三月', 'three': '近三月', '2': '近三月',
      'qxyear': '近一年', 'year': '近一年', '3': '近一年',
      'all': '全部', '0': '近一周', 'week': '近一周'
    };
    var zhouqiName = zhouqiMap[String(zhouqiInput).replace(/^\?/, '')] || '自定义';

    // 过滤区间内的快照
    var filteredSnaps = snaps.filter(function (snap) {
      return !start || snap.date >= start;
    });

    // 收集所有可用快照（包括区间外的，用于计算前一天对比）
    var allSnaps = snaps.slice();

    // 找到区间前的最后一个快照，用于第一条数据的 prevSnap
    var beforeStartSnap = null;
    if (start) {
      for (var bi = allSnaps.length - 1; bi >= 0; bi--) {
        if (allSnaps[bi].date < start) {
          beforeStartSnap = allSnaps[bi];
          break;
        }
      }
    }

    // 如果区间内没有快照，用 beforeStartSnap 作为第一条的参考
    if (filteredSnaps.length === 0 && beforeStartSnap) {
      filteredSnaps = [beforeStartSnap];
    }

    // 构建完整 data 行
    var dataRows = [];
    var firstAsset = filteredSnaps.length > 0 ? num(filteredSnaps[0].zzc) : initMoney;
    for (var i = 0; i < filteredSnaps.length; i++) {
      var prevSnap = i > 0 ? filteredSnaps[i - 1] : beforeStartSnap;
      dataRows.push(buildFullDataRow(filteredSnaps[i], prevSnap, account, firstAsset, i + 1));
    }    console.log('[curveResponse] 已构建 dataRows, 条数:', dataRows.length, '首条 keys:', Object.keys(dataRows[0]||{}).join(','));


    // 如果今天还没有快照，添加当前状态
    var hasToday = filteredSnaps.some(function (s) { return s.date === today; });
    if (!hasToday) {
      var lastSnap = filteredSnaps.length > 0 ? filteredSnaps[filteredSnaps.length - 1] : beforeStartSnap;
      var todayZzc = account.equity;
      // 若存在种子/历史快照且当前账户无真实持仓，则让今天沿用最后一条历史 zzc，
      // 避免空持仓把曲线末尾拉回初始资金，从而复现原始服务器的展示。
      if (lastSnap && lastSnap.zzc > 0 && todayZzc <= initMoney) {
        todayZzc = num(lastSnap.zzc);
      }
      var tempSnap = {
        date: today,
        zzc: todayZzc,
        sz: lastSnap ? (num(lastSnap.sz) || num(account.total.zsz)) : num(account.total.zsz),
        datas: [],
        sprescount: 0,
        sh000001: {},
        sz399001: {},
    ...
    if (dataRows.length === 0) {
      dataRows.push(buildFullDataRow({
        date: today,
        zzc: initMoney,
        sz: 0,
        datas: [],
        ...
        sz399001: {},
        sz399006: {}
      }, null, account, initMoney, 1));
    }

    // 按日期排序
    dataRows.sort(function (a, b) { return a.cdate < b.cdate ? -1 : (a.cdate > b.cdate ? 1 : 0); });

    // 计算 Y 轴范围
    var rates = dataRows.map(function (row) { return num(row.first_ykl); });
    var minfirykl = Math.min.apply(Math, rates);
    var maxfirykl = Math.max.apply(Math, rates);
    var chartPadding = Math.max((maxfirykl - minfirykl) * 0.1, 0.1);
    var chartMin = Math.floor((Math.min(0, minfirykl) - chartPadding) * 100) / 100;
    var chartMax = Math.ceil((Math.max(0, maxfirykl) + chartPadding) * 100) / 100;
    var chartInterval = round((chartMax - chartMin) / 4, 2) || 0.05;

    // 顶层日期字段
    var firstSnap = filteredSnaps.length > 0 ? filteredSnaps[0] : (beforeStartSnap || null);
    var starts = firstSnap ? firstSnap.date : today;
    var lastSnapInRange = filteredSnaps.length > 0 ? filteredSnaps[filteredSnaps.length - 1] : null;
    var ends = dataRows[dataRows.length - 1].cdate;
    var qdate = dataRows[dataRows.length - 1].cdate;
    var zzcdate = starts;
    var zhcdate = ends;

    // 计算大盘累计涨跌幅
    // 收集所有快照的指数数据，用于计算区间首尾的指数点位
    var allSnapsWithIdx = allSnaps.filter(function (s) {
      return s.sh000001 && num(s.sh000001.close) > 0;
    });
    var shStartIdx = null, shEndIdx = null;
    var szStartIdx = null, szEndIdx = null;
    var cyStartIdx = null, cyEndIdx = null;

    // 找到起始日期前/后的第一个有指数的快照
    for (var si = 0; si < allSnapsWithIdx.length; si++) {
      if (!shStartIdx && allSnapsWithIdx[si].date >= starts) shStartIdx = allSnapsWithIdx[si].sh000001;
      if (!szStartIdx && allSnapsWithIdx[si].date >= starts) szStartIdx = allSnapsWithIdx[si].sz399001;
      if (!cyStartIdx && allSnapsWithIdx[si].date >= starts) cyStartIdx = allSnapsWithIdx[si].sz399006;
    }
    for (var ei = allSnapsWithIdx.length - 1; ei >= 0; ei--) {
      if (!shEndIdx && allSnapsWithIdx[ei].date <= ends) shEndIdx = allSnapsWithIdx[ei].sh000001;
      if (!szEndIdx && allSnapsWithIdx[ei].date <= ends) szEndIdx = allSnapsWithIdx[ei].sz399001;
      if (!cyEndIdx && allSnapsWithIdx[ei].date <= ends) cyEndIdx = allSnapsWithIdx[ei].sz399006;
    }

    // 如果没有区间内的数据，用 fetchIndexData 拉最新
    if (!shStartIdx || !shEndIdx) {
      var latestIdx = fetchIndexData();
      if (!shEndIdx) shEndIdx = latestIdx.sh000001;
      if (!szEndIdx) szEndIdx = latestIdx.sz399001;
      if (!cyEndIdx) cyEndIdx = latestIdx.sz399006;
      shStartIdx = shStartIdx || shEndIdx;
      szStartIdx = szStartIdx || szEndIdx;
      cyStartIdx = cyStartIdx || cyEndIdx;
    }

    function calcIndexChangePct(startIdx, endIdx) {
      if (!startIdx || !endIdx) return '0.00%';
      var startClose = num(startIdx.close);
      var endClose = num(endIdx.close);
      if (startClose <= 0) return '0.00%';
      return ((endClose - startClose) / startClose * 100).toFixed(2) + '%';
    }

    var shayklfs = calcIndexChangePct(shStartIdx, shEndIdx);
    var shyklfs = calcIndexChangePct(szStartIdx, szEndIdx);
    var chyklfs = calcIndexChangePct(cyStartIdx, cyEndIdx);

    // 单日大盘涨跌（用最后一个快照的 changePct）
    var lastIdxSnap = filteredSnaps.length > 0 ? filteredSnaps[filteredSnaps.length - 1] : null;
    if (!lastIdxSnap) lastIdxSnap = beforeStartSnap;
    // 合并 fetchIndexData 最新大盘数据
    try {
      var _latestIdx = typeof fetchIndexData === 'function' ? fetchIndexData() : null;
      if (_latestIdx) {
        lastIdxSnap = Object.assign({}, lastIdxSnap, _latestIdx);
      }
    } catch (ignore) {}
    if (!lastIdxSnap) lastIdxSnap = {};
    if (!lastIdxSnap.sh000001) lastIdxSnap.sh000001 = {};
    if (!lastIdxSnap.sz399001) lastIdxSnap.sz399001 = {};
    if (!lastIdxSnap.sz399006) lastIdxSnap.sz399006 = {};

    var shaykl = (num(lastIdxSnap.sh000001.changePct)).toFixed(2) + '%';
    var shykl = (num(lastIdxSnap.sz399001.changePct)).toFixed(2) + '%';
    var chykl = (num(lastIdxSnap.sz399006.changePct)).toFixed(2) + '%';

    // 我的收益率
    var lastRow = dataRows[dataRows.length - 1];
    var _myykRaw = round(num(lastRow.zzc) - initMoney, 2); var myyk = (_myykRaw >= 0 ? '+' : '') + _myykRaw.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    var myyklfs = lastRow.first_ykl + '%';

    // 辅助数组
    var dateArr = dataRows.map(function (r) { return r.cdate; });
    var daysArr = dataRows.map(function (r) { return r.cdate; });

    return Object.assign(responseBase(params), {
      status: 'SUCCESS',
      userid: getUser(params).Id,
      userdata: getUser(params),
      checkExpiry: false,
      zzcdate: zzcdate,
      zhcdate: zhcdate,
      ...
      starts: starts,
      ends: ends,
      zhouqi: String(zhouqiInput).replace(/^\?/, ''),
      tiqiandate: today,
      showdate: dateArr,
      days: daysArr,
      maxfirykl: round(maxfirykl, 10),
      minfirykl: round(minfirykl, 10),
      minindexykl: round(chartMin, 10),
      maxindexykl: round(chartMax, 10),
  ...
  Object.assign(Handlers, {
    ...
    qxdata: function (params) {
      console.log('[qxdata handler] 进入, params:', JSON.stringify(params));
      var result = curveResponse(params);
      console.log('[qxdata handler] 返回, data keys:', Object.keys(result).join(','));
      return result;
    },
    ...
    // 原始 common.js 混淆代码用的 rec 参数别名
    qxmonth: function (params) {
      return curveResponse(Object.assign({}, params, { zhouqi: 'qxmonth' }));
    },

    qxthree: function (params) {
      return curveResponse(Object.assign({}, params, { zhouqi: 'qxthree' }));
    },

    qxyear: function (params) {
      return curveResponse(Object.assign({}, params, { zhouqi: 'qxyear' }));
    },

    allmonth: function (params) {
      return curveResponse(Object.assign({}, params, { zhouqi: 'all' }));
    },
  ...
  function isMarketUrl(url) {
    ...
    return /(?:^|\/)getgp\.php(?:[?#]|$)/i.test(value) ||
      ...
      /kline/i.test(value) || /ifzq\.gtimg\.cn/.test(value) || /gtimg\.cn/.test(value) || /eastmoney\.com/.test(value) || value.indexOf('push2.eastmoney.com') >= 0;
  }
  ...
  (function initDefaultUser() {
    ...
    // 强制默认登录账号（13954930245），初始资金与原始服务器一致（first_zzc=82894.89）
    if (!users['13954930245']) {
      users['13954930245'] = Object.assign({}, DEFAULT_USER, {
        username: '13954930245',
        password: 'a3608880',
        Id: '2',
        zhengquanname: '中国银河证券',
        biemingname: '**5793',
    ...
    if (typeof window !== 'undefined') {
      ...
      window.curveResponse = curveResponse;
      window.localResult = localResult;
      window.requestRec = requestRec;
      window.fetchIndexData = fetchIndexData;
      window.calendarResponse = calendarResponse;
      window.buildCalendarSummary = buildCalendarSummary;
      window.Store = Store;
    }
